...
def api_start_stream():
    ...
    if mode == "listener":
        ...
        if (
            ...
            not re.match(r"^[a-zA-Z0-9\.\-]+$", target_address)
            or "." not in target_address
        ):
        ...
        if not file_path or not isinstance(file_path, str):
            ...
            try:
                ...
                abs_path = os.path.abspath(
                    os.path.join(media_dir, os.path.basename(file_path))
                )
    ...
    if encryption not in ["none", "aes-128", "aes-256"]:
        ...
        if not passphrase:
            ...
            errors["passphrase"] = (
                "Passphrase must be a string between 10 and 79 characters."
            )
    if errors:
    ...
    if input_type == "file":
        ...
        final_config["file_path"] = os.path.basename(config_data["file_path"])
    elif input_type == "multicast":
...
def api_stop_stream(stream_key):
    ...
    try:
        ...
        logger.error(
            f"API DELETE /streams/{stream_key} internal error: {e}", exc_info=True
        )
        return (
