...
$(document).ready(function () {
  ...
  form.submit(function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.error && data.rtt_ms === null) {
          // Check for fallback/total failure case
          ...
          $("#recommendation-note")
            .text(
              data.error ||
                "Recommendations based on Haivision SRT Guide for measured conditions.",
            )
            .removeClass("text-danger text-warning text-muted")
            .addClass(
              data.error && !data.error.toLowerCase().includes("estimated")
                ? "text-warning"
                : "text-muted",
            ); // Warning if non-estimated error msg exists
        }
      },
      error: function (xhr) {
        ...
        if (xhr.responseJSON) {
          if (xhr.responseJSON.error && xhr.responseJSON.details) {
            let detailMsg =
              typeof xhr.responseJSON.details === "object"
                ? Object.entries(xhr.responseJSON.details)
                    .map(([f, m]) => `${f}: ${m}`)
                    .join("; ")
      ...
      complete: function () {
        ...
        $("#run-test-btn")
          .prop("disabled", false)
          .html('<i class="fas fa-play-circle"></i> Run Network Test');
      },
    });
  });
  ...
  function displayResults(data) {
    ...
    if (data.loss_percent !== null) {
      ...
      data.bandwidth_type === "TCP" ||
      data.bandwidth_type === "N/A" ||
      data.bandwidth_type === null
    ) {
    ...
    $("#result-bandwidth").text(
      data.bandwidth_mbps ? data.bandwidth_mbps + " Mbps" : "N/A",
    );
    const bwElem = $("#result-bandwidth-type");
    if (data.bandwidth_type && data.bandwidth_type !== "N/A") {
      bwElem
        .text(data.bandwidth_type)
        .removeClass("d-none bg-secondary bg-info bg-warning")
        .addClass(data.bandwidth_type === "TCP" ? "bg-info" : "bg-warning"); // Orange for UDP, Blue for TCP
    } else {
      bwElem.addClass("d-none");
    }
    $("#result-latency").text(
      data.latency_recommendation !== null ? data.latency_recommendation : "-",
    );
    $("#result-multiplier-inline").text(
      data.rtt_multiplier ? data.rtt_multiplier + "x" : "-",
    );
    $("#result-overhead").text(
      data.overhead_recommendation !== null
        ? data.overhead_recommendation
        : "-",
    );
    $("#loss-percent-inline").text(
      data.loss_percent !== null ? data.loss_percent.toFixed(2) : "-",
    );
    ...
    $("html, body").animate(
      { scrollTop: $("#test-results").offset().top - 20 },
      500,
    );
  }
  ...
  function updateProgressBars(data) {
    const latP =
      data.latency_recommendation !== null
        ? Math.min(100, (data.latency_recommendation / 1000) * 100)
        : 0; // Example scaling
    $("#latency-bar")
      .css("width", latP + "%")
    ...
    const ovP =
      data.overhead_recommendation !== null
        ? Math.min(100, data.overhead_recommendation)
        : 0; // Overhead is already %
    $("#overhead-bar")
      .css("width", ovP + "%")
      .removeClass("bg-success bg-warning bg-danger bg-info")
      .addClass("bg-success");
  ...
  function setQualityIndicators(data) {
    $(
      "#result-rtt, #result-loss, #result-jitter, #result-latency, #result-overhead",
    ).removeClass("good-value moderate-value poor-value");
    let rttC = "good-value";
