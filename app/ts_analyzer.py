...
class TSAnalyzer:
            ...
            try:
                ...
                self.analyzers[port] = {
                    ...
                    "pid_stats": defaultdict(
                        lambda: {"packets": 0, "last_cc": -1, "cc_errors": 0}
                    ),
                    "program_info": {},
                    ...
                    for signal in ["pat-info", "pmt-info", "sdt-info"]:
                        try:
                            if hasattr(demux, "connect") and signal in dir(demux):
                                demux.connect(
                                    signal,
                                    getattr(self, f'_on_{signal.replace("-", "_")}'),
                                    port,
                                )
                        except Exception as e:
                            self.logger.warning(
                                f"Could not connect to {signal} signal: {e}"
                            )
        ...
        with self.lock:
            if port in self.analyzers:
                ...
                if len(analyzer["table_repetition_rates"]["PAT"]) >= 2:
                    ...
                    analyzer["pat_interval"] = (
                        sum(intervals) / len(intervals) * 1000
                    )  # ms
    ...
    def get_ts_analysis(self, port):
        ...
        with self.lock:
            ...
            if (
                analyzer["pat_interval"] > 500
            ):  # DVB standard: PAT should repeat every 500ms
                analysis["compliant"] = False
                analysis["issues"].append(
                    f"PAT interval too high: {analyzer['pat_interval']:.2f}ms (should be ≤500ms)"
                )

            if analyzer["pmt_interval"] > 500:  # PMT should repeat every 500ms
                analysis["compliant"] = False
                analysis["issues"].append(
                    f"PMT interval too high: {analyzer['pmt_interval']:.2f}ms (should be ≤500ms)"
                )

            if analyzer["sdt_interval"] > 2000:  # SDT should repeat every 2s
                analysis["compliant"] = False
                analysis["issues"].append(
                    f"SDT interval too high: {analyzer['sdt_interval']:.2f}ms (should be ≤2000ms)"
                )

            if analyzer["continuity_errors"] > 0:
                analysis["compliant"] = False
                analysis["issues"].append(
                    f"Continuity counter errors: {analyzer['continuity_errors']}"
                )

            if analyzer["pcr_jitter_ms"] > 30:  # PCR jitter should be minimal
                analysis["compliant"] = False
                analysis["issues"].append(
                    f"PCR jitter too high: {analyzer['pcr_jitter_ms']:.2f}ms"
                )
